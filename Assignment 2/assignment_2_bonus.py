...
def preprocess_data(data):
        ...
        #random_translation= albu.ShiftScaleRotate(shift_limit=0.5, scale_limit=0.5, rotate_limit=20, p=0.5)
        #X = X.reshape(10000, 3, 32, 32)
        #X = X.transpose(0,2,3,1).astype("uint8")
        #X = [flip_images(image=x)["image"] for x in X]
        #X = [random_translation(image=x)["image"] for x in X]
...
def apply_random_mirror(X):
    # Randomly mirror images horizontally
    mirror_indices = np.random.choice([-1, 1], X.shape[0])
    X_mirrored = X * mirror_indices[:, np.newaxis, np.newaxis]
    return X_mirrored

def apply_random_translation(X):
    # Randomly translate images
    tx = random.randint(-3, 3)
    ty = random.randint(-3, 3)

    aa = np.arange(32)
    vv = np.tile(32*aa, (32-abs(tx), 1))
    bb1 = np.arange(tx+1, 33)
    bb2 = np.arange(1, 33-abs(tx))
    
    ind_fill = vv.ravel() + np.tile(bb1[:, np.newaxis], (32, 1))
    ind_xx = vv.ravel() + np.tile(bb2[:, np.newaxis], (32, 1))

    # Adjust indices for negative translations
    if tx < 0:
        ind_fill = ind_fill[-tx*32:]
        ind_xx = ind_xx[:-tx*32]
    elif tx > 0:
        ind_fill = ind_fill[:-tx*32]
        ind_xx = ind_xx[tx*32:]
    ...
    if ty < 0:
        ind_fill = ind_fill[ind_fill >= -ty*32+1]
        ind_xx = ind_xx[ind_xx <= 1024-ty*32]
    elif ty > 0:
        ind_fill = ind_fill[:-(ty*32)]
        ind_xx = ind_xx[:(1024-ty*32)]
    ...
    X_translated[:, inds_fill] = X[:, inds_xx]
...
def MiniBatchGDCyclicDropout(X_train, Y_train, y_train, X_val, Y_val, y_val, W_1, b_1, W_2, b_2, lambda_, n_batch, eta, n_epochs, dropout_prob):
    ...
    for cycle in range(cycles):
        for step in range(2*n_s):
            ...
            s1 = np.dot(W_1, X_batch) + b_1
            h = np.maximum(0, s1)
            if dropout_prob > 0:  
                mask = np.random.rand(*h.shape) < dropout_prob
                h *= mask / dropout_prob
            ...
            G = - (Y_batch - p)
            grad_W2 = G @ h.T / batch_size + 2 * lambda_ * W_2
            grad_b2 = np.sum(G, axis=1).reshape(10, 1) / batch_size

            G = np.dot(W_2.T, G)
            G = G * (h > 0)

            grad_W1 = (1 / batch_size) * np.dot(G, X_batch.T) + 2 * lambda_ * W_1
            grad_b1 = (1 / batch_size) * np.sum(G, axis=1, keepdims=True) 
...
results_dict = MiniBatchGDCyclicDropout(X_train, Y_train, y_train, X_val, Y_val, y_val, W1, b1, W2, b2, lambda_, 100, 0.01, 10, dropout_prob)
